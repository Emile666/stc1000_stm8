...
|cF|Celsius or Fahrenheid display|0 = Celsius, 1 = Fahrenheit|
...
**Temperature correction**, will be added to the temperature sensor, this allows the user to calibrate the temperature reading. It is best to calibrate with a precision resistor of 10 k Ohms (1% tolerance). Replace the temperature sensor with such a resistor and adjust this parameter
such that the display is set to 25.0 °C.

**Temperature correction 2**, same as *temperature correction*, but for the second temperature probe. If you remove the existing 2-pin connector, you can replace it with a 3-pin connector. The 1st temperature sensor is connected to pins 1 and 2, the 2nd temperature sensor can be connected between 2 and 3. Enable this 2nd sensor with the *Pb2* parameter.
...
**Celsius or Fahrenheit** can be used to set display mode for temperatures to Celsius or Fahrenheit. Note that the values in EEPROM are not directly changed with it, so if you change this, you need to make sure that the stored parameters are updated!

**Second temperature probe** can be used to enable the second temperature probe. See below for a more detailed explanation.

**Hours or Minutes** can be used to select between a time-mode in hours or minutes. Typically the minutes time-mode is selected when you want to control a brewing-session. When controlling a fermentation in a climate chamber, you typically select hours.

**Hc**, this is the proportional gain for the PID controller. See below for a more detailed explanation.

**Ti**, this is the integral time-constant for the PID controller. See below for a more detailed explanation.
...
simulate the 2nd temperature probe. If you need an additional probe, what you are looking for is a 10k NTC probe with a beta value of 3435 (I'm guessing that is between 25-85C, but the retailers rarely specify). 
...
STC-1000+-STM8 is written in C and compiled using IAR STM8 embedded workbench
...
* The IAR STM8 Integrated Development Environment (IDE) has a code-size limit of 8 Kbytes. However, this is sufficient for our STM8 uC.

* A separate scheduler (non pre-emptive) has been added to address all timing issues.

* Hardware routines (interrupts, ADC, eeprom) have all been rewritten, other routines have been copied and adapted from the stc1000p github repository.
