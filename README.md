...
- **Refrigerator mode with compressor fan control**: If you connect a second temperature probe and set the **Pb2** parameter to 2, you can control a refrigerator and the compressor fan at the same time. The first temperature probe goes inside the refrigerator as usual, the second one is connected to one of the compressor output pipes. The cooling output controls the refrigerator itself, the heating output is now used for the compressor fan. I am using this to control a keg-cooler (replacing the stock controller for a STC1000).
